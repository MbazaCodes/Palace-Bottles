...
export default function Categories() {
  ...
  useEffect(() => {
    Promise.all([getCategories(), getProducts()]).then(([c, p]) => { setCategories(c); setProducts(p); });
  }, []);
  return (
    <section id="categories" className="mx-auto max-w-7xl px-4 py-16">
      ...
      {CATEGORIES.length === 0 && <p className="mt-6 text-center text-sm text-navy/50">No categories yet. Add categories from the admin panel.</p>}
          ...
          return (
            <motion.div key={c.slug}
              ...
              <Link href={`/shop?category=${c.slug}`}
                ...
                {sample && <Bottle3D {...sample.visual} className="mx-auto w-3/5 transition-transform duration-300 group-hover:-translate-y-1.5 group-hover:scale-105" label={false} />}
