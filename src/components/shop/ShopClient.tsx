  ...
  return (
    <div className="mx-auto grid max-w-7xl gap-6 px-4 py-8 lg:grid-cols-[16rem_1fr]">
      <aside className={`${filtersOpen ? "block" : "hidden"} h-fit rounded-2xl border border-silver bg-white p-5 shadow-card lg:block`}>
        ...
        <fieldset className="mt-5"><legend className="text-sm font-bold text-navy">Categories</legend>
        ...
        <fieldset className="mt-6"><legend className="text-sm font-bold text-navy">Capacity</legend>
          <div className="mt-3 space-y-2.5 text-sm">
            {CAPACITIES.map((c) => (<label key={c} className="flex cursor-pointer items-center gap-2"><input type="checkbox" checked={caps.includes(c)} onChange={() => toggleCap(c)} className="accent-royal" /> {c}</label>))}
          </div>
        </fieldset>
        <fieldset className="mt-6"><legend className="text-sm font-bold text-navy">Price Range</legend>
          <input type="range" min={10000} max={120000} step={1000} value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} className="mt-3 w-full accent-royal" />
          <div className="flex justify-between text-xs text-navy/55"><span>TZS 10,000</span><span>Up to TZS {maxPrice.toLocaleString()}</span></div>
        </fieldset>
      </aside>
      <div>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-navy/65">{loaded ? `Showing ${filtered.length} of ${products.length} products` : "Loading products..."}</p>
          <div className="flex items-center gap-2">
            <button onClick={() => setFiltersOpen(!filtersOpen)} className="rounded-xl border border-silver bg-white px-3 py-2 text-sm font-semibold text-navy lg:hidden">Filters</button>
            <select value={sort} onChange={(e) => setSort(e.target.value)} className="rounded-xl border border-silver bg-white px-3 py-2 text-sm font-semibold text-navy outline-none">
              <option value="recommended">Recommended</option><option value="price-asc">Price: Low → High</option><option value="price-desc">Price: High → Low</option><option value="rating">Top Rated</option>
            </select>
          </div>
        </div>
        {loaded && filtered.length === 0 ? (
          <p className="mt-16 text-center text-sm text-navy/55">No products yet. Add products from the admin panel.</p>
        ) : (
